...
namespace MojehraDroid
{
    ...
    {        
        ...
        private TouchCollection doteky;
        ...
        protected override void Initialize()
        {
            ...
            Vibrace.Init((Vibrator)Activity.GetSystemService(Android.Content.Context.VibratorService));
                ...
                else
                {
                    ...
                    if (splashScreen.ProvedUpdate)
                    {
                        ...
                        if (gameState == Stavy.Play)
                        {
                            ...
                            {
                                player.BludisteInput(Dotek(doteky));
        ...
        private Point Dotek(TouchCollection touchCollection)
        {
            ...
            if (zadanyPohyb != predchoziPohyb) 
                return zadanyPohyb.ToPoint();
       
            return Point.Zero;
        }

        private void ProjdiStavy(TouchCollection doteky)
        {
            if (gameState == Stavy.Prohra)
            {
                if (delkaAnimaceHrace == 0)
                {
                    ...
                    if (Store.SkoreTotal > 0)
                    {
                        if (waitFrames % 2 == 0)
                        {
                            Store.SkoreTotal--;
                            skoreTotalString = Store.SkoreTotal.ToString();
                        }
                    }
                    if (doteky.Count > 0)
                    {
        ...
        private void Zvitezit()
        {
            ...
            if (delayedBonus > 0)
                NapisVelkouZpravu($"You Win{System.Environment.NewLine}Delayed Bonus: {delayedBonus}", 10000, -9999, -9999, true, true, Color.Aqua);
            else
